...
數位產品。手機優先的電腦組裝估價單前端雛形：以前端靜態頁面載入 CSV 資料，完成零件挑選、相容性提示、進度總覽與估價單儲存流程，之後可將 `data/products.csv` 的資料入口替換為正式 API。介面美術與互動依《[規格書](規格書.txt)》、[`PRODUCT.md`](PRODUCT.md) 與 ** PCShop 設計系統** 實作，定調為「沉穩、精準、守護」的新手友善風格。
...
286- 組裝進度、總價、軟警告、估價單儲存、載入、刪除、複製與列印。
